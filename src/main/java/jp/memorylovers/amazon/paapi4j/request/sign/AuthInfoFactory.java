...
public class AuthInfoFactory {
    ...
    public static AuthInfo createByResoruce() throws PAAPI4jException {
        ...
        if (!rb.containsKey(KEY_ACCESS_KEY) || !rb.containsKey(
            KEY_SECRET_KEY)) {
            String msg = "There is no " + KEY_ACCESS_KEY + " and/or " + KEY_SECRET_KEY + " property values " + "in " + RESOURCE_NAME + ".properties";
            throw new PAAPI4jException(msg);
        }

        AuthInfo info = new AuthInfo();

        if (rb.containsKey(KEY_ASSOCIATE_TAG) && !rb.getString(KEY_ASSOCIATE_TAG).isEmpty()) {
